from src.constants import XP_PROGRESSION
import random
...
def calculate_armor_class(class_name, attribute_values):
    ...
    if class_name == "Barbarian":
        ...
        return 10 + dexterity_modifier + constitution_modifier  # Barbarian AC formula 
        ...
        return 10 + dexterity_modifier  # Default AC formula 
...
def calculate_hit_points(class_name, level, constitution_modifier, use_average=False):
    """
    Calculate hit points based on class, level, and constitution modifier.
    If use_average is True, use average HP per level. Otherwise, simulate rolling hit dice.
    """
    # Define hit die types for each class
    hit_die_types = {
        "Barbarian": 12,
        "Fighter": 10,
        "Paladin": 10,
    ...
    # Get the hit die for the given class (default to d8 if class is unknown)
    hit_die = hit_die_types.get(class_name, 8)

    if use_average:
        # Use average HP per level: (hit_die // 2) + 1
        average_hp_per_level = (hit_die // 2) + 1
        total_hp = average_hp_per_level * level + (constitution_modifier * level)
    else:
        # Simulate rolling hit dice for each level
        total_hp = 0
        for _ in range(level):
            roll = random.randint(1, hit_die)  # Simulate rolling the hit die <button class="citation-flag" data-index="1">
            total_hp += roll
        total_hp += constitution_modifier * level  # Add constitution modifier for all levels
